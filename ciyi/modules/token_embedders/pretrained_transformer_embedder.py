...
@TokenEmbedder.register("pretrained_transformer_layern")
class PretrainedTransformerEmbedder(TokenEmbedder):
    ...
    def __init__(
            ...
            *,
            ...
            transformer_layer: Union[int, str] = None,
        ...
        if not last_layer_only:
            if transformer_layer is None:
                ...
                self.transformer_layer = transformer_layer if self.transformer_layer == 'word_embeddings' else int(
                    transformer_layer)
            self.config.output_hidden_states = True
        else:
            assert transformer_layer is None
    ...
    def forward(
        ...
        elif self.transformer_layer is not None:
            if isinstance(self.transformer_layer, int):
                hidden_states = transformer_output[-1]
                embeddings = hidden_states[self.transformer_layer]
            else:
                if self.transformer_layer == 'word_embeddings':
                    embeddings = self.transformer_model.embeddings.word_embeddings(token_ids)
                else:
                    assert f"Transformer layer {self.transformer_layer} is not supported!"
        else:
