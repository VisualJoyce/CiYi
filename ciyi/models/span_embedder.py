...
class SpanEmbedder(SpanEmbedderBase):
...
@Model.register("sentence_span_embedder_mean")
class SentenceSpanEmbedderMean(SpanEmbedderBase):
    ...
    def forward(self,  # type: ignore
        ...
        sim_sent = torch.cosine_similarity(embedded_sent1, embedded_sent2)
        sim_span = torch.cosine_similarity(embedded_span1, embedded_span2)
        sim = (sim_span + sim_sent) / 2
        output_dict["sim"] = sim
        if label is not None:
            self._correlation(sim, label)
            loss = self._loss(sim, label)
...
@Model.register("sentence_span_embedder_concat")
class SentenceSpanEmbedderConcat(SpanEmbedderBase):
    ...
    def forward(self,  # type: ignore
        ...
        sim = torch.cosine_similarity(torch.cat([embedded_sent1, embedded_span1], dim=-1),
                                      torch.cat([embedded_sent2, embedded_span2], dim=-1))
        output_dict["sim"] = sim
        if label is not None:
            self._correlation(sim, label)
            loss = self._loss(sim, label)
